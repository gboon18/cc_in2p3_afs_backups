{
  ...
  string dir_str = "/sps/clas/hosanko/NPS/geant4.10.03.p01_wMT/magnetic/01052018/rootfiles/";
  ...
  TH1F *h_dose_on = new TH1F("h_dose_on", "field on", 31, 0, 30);//in x axis
  h_dose_on->GetXaxis()->SetTitle("angle");
  h_dose_on->GetYaxis()->SetTitle("dose rate [rad/hr]");
  TH1F *h_dose_off = new TH1F("h_dose_off", "field off", 31, 0, 30);//in x axis
  ...
  for(int i = 0 ; i < ch_on1->GetEntries() ; i++){
    ...
    for(int j = 0 ; j < array ; j++){
      ...
      cout<<"Entry, edep_on[Array] : "<<i<<", "<<edep_on1[j]<<"["<<j<<"]"<<endl;
      /* cout<<"edep_off : "<<edep_off1[j]<<endl; */
      h_dose_on->Fill(x, edep_on1[j]);
      h_dose_off->Fill(x, edep_off1[j]);
    }
  } 

  TCanvas *c1 = new TCanvas("c1", "", 200, 10, 1400, 500);
  c1->Divide(2,1);
  c1->cd(1);
  h_dose_on->Draw();
  c1->cd(2);
