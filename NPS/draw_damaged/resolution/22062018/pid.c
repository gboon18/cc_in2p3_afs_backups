{
  ...
  string dir_str = "/sps/clas/hosanko/NPS/geant4.10.03.p01_wMT/magnetic/22062018/rootfiles/";
  ...
  TH2F *h_elec_pattern_on = new TH2F("h_elec_pattern_on", "field on, electron",31, 0, 30, 36, 0, 35);
  TH2F *h_gamma_pattern_on = new TH2F("h_gamma_pattern_on", "field on, gamma",31, 0, 30, 36, 0, 35);
  TH2F *h_elec_pattern_off = new TH2F("h_elec_pattern_off", "field off, electron",31, 0, 30, 36, 0, 35);
  TH2F *h_gamma_pattern_off = new TH2F("h_gamma_pattern_off", "field off, gamma",31, 0, 30, 36, 0, 35);
  ...
  for(int i = 0 ; i < ch_on1->GetEntries() ; i++){
    ...
    for(int j = 0 ; j < array ; j++){
      ...
      if(pid_on1[j] == 11) {/* cout<<"working"<<endl; */ h_elec_pattern_on->Fill(x, y, ene_on1[j]);}
      if(pid_on1[j] == 22) h_gamma_pattern_on->Fill(x, y, ene_on1[j]);
      if(pid_off1[j] == 11) {cout<<ene_off1[j]<<"["<<j<<"]"<<endl; h_elec_pattern_off->Fill(x, y, ene_off1[j]);}
      if(pid_off1[j] == 22) h_gamma_pattern_off->Fill(x, y, ene_off1[j]);
    }
  } 
