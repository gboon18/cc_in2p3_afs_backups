{
  ...
  int x, y;
  int No_event = 100*1e7;//each file containes 1e7 events and we have 100 files per each settings.
  double angle;
  ...
  c_flux_pid->cd(2);
  gPad->SetLogz(1);
  h_pos_pattern_on->GetZaxis()->SetRangeUser(1,1e7);
  ...
  c_flux_pid->cd(4);
  gPad->SetLogz(1);
  h_unknown_pattern_on->GetZaxis()->SetRangeUser(1,1e7);
  ...
  c_flux_pid->cd(6);
  gPad->SetLogz(1);
  h_pos_pattern_off->GetZaxis()->SetRangeUser(1,1e7);
  ...
  c_flux_pid->cd(8);
  gPad->SetLogz(1);
  h_unknown_pattern_off->GetZaxis()->SetRangeUser(1,1e7);
  h_unknown_pattern_off->Draw("colz");

  c_ene->cd(); 
  gPad->SetLogy(1); 

  h_ene_off->SetMaximum(1e8); 
  h_ene_off->SetMinimum(1e3); 
  ...
  c_ene_pid->cd(3); 
  gPad->SetLogy(1); 
  h_gamma_ene_off->SetMaximum(1e8); 
  ...
  c_ene_pid->cd(4); 
  gPad->SetLogy(1); 
  h_unknown_ene_off->SetMaximum(1e8); 
